...
export default function Aggregate_Table({ data, title }: AggTableProps) {
  ...
  const users = [
    ...new Set([...Object.keys(data[0]).filter(k => k !== "date_queried"), ...devUserIds]),
  ];
    ...
    >
      <thead>
        <tr style={{ backgroundColor: "#007bff", color: "white" }}>
          <th style={{ padding: "8px", border: "1px solid #ddd", position: "sticky", top: 0 }}>
            Date
          </th>
          {users.map(u => (
            <th
              key={u}
              style={{ padding: "8px", border: "1px solid #ddd", position: "sticky", top: 0 }}
            >
              {u}
            </th>
          ))}
      ...
      <tbody>
        {data.map(row => (
          <tr key={row.date_queried}>
            ...
            {users.map(u => (
              <td key={u} style={{ padding: "6px", border: "1px solid #ddd", textAlign: "center" }}>
                {row[u] ?? ""}
              </td>
            ))}
          </tr>
